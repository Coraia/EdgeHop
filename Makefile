...
build-server:
	go build -trimpath -ldflags="-s -w" -o bin/uc-server ./cmd/uc-server

build-client:
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o bin/uc-client-linux-amd64 ./cmd/uc-client
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o bin/uc-client-linux-arm64 ./cmd/uc-client
